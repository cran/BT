...
testthat::test_that("Check the BT_CV_Errors and BT_CV_Predict functions",{
  ...
  # Create folds.
  trainFrac <- 0.6
  folds <- c(rep(1, n*trainFrac/3), rep(2, n*trainFrac/3), rep(3, n*trainFrac/3))
  # Run BT algos to simulate the CV results.
  datasetReduce <- datasetFull[seq(1, trainFrac*n),]
  firstOrder <- c(which(folds == 2), which(folds == 3), which(folds == 1))
  secondOrder <- c(which(folds == 1), which(folds == 3), which(folds == 2))
  thirdOrder <- c(which(folds == 1), which(folds == 2), which(folds == 3))
  ...
  BT_CV_toCheck <- BT(formula = as.formula("Y_normalized ~ Age + Sport + Split + Gender"),
                      data = datasetFull,
                      tweedie.power = 1,
                      ABT = T,
                      n.iter = 200,
  ...
  cv_error_recomputed <- (1/(n*trainFrac))*(BT_algo1$BTErrors$validation.error*(n*trainFrac/3) +
                                              BT_algo2$BTErrors$validation.error*(n*trainFrac/3) +
                                              BT_algo3$BTErrors$validation.error*(n*trainFrac/3))
  min_cv_error_recomputed <- which.min(cv_error_recomputed)
  pred_BT_algo1 <- predict(BT_algo1, newdata = datasetReduce[which(folds==1),], n.iter = min_cv_error_recomputed, type = "link")
  pred_BT_algo2 <- predict(BT_algo2, newdata = datasetReduce[which(folds==2),], n.iter = min_cv_error_recomputed, type = "link")
  pred_BT_algo3 <- predict(BT_algo3, newdata = datasetReduce[which(folds==3),], n.iter = min_cv_error_recomputed, type = "link")
  pred_BT_recomputed <- c(pred_BT_algo1, pred_BT_algo2, pred_BT_algo3)
