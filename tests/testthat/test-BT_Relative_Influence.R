...
testthat::test_that("Check the BT_Relative_Influence function - Results - Without surrogates",{
  ...
  importance2 <- function(fit)
  {
    ...
    scaled.imp <- if (fit$method == "anova") fit$splits[spri, "improve"] * ff$dev[fpri] else fit$splits[spri, "improve"]
  ...
  for (iTree in seq_len(n.iter)){
    ...
    for (colName in varName){
      if (colName %in% names(currRes)){
        resMatrix[iTree, colName] <- unname(currRes[names(currRes)==colName])
      }
    }
  }
...
testthat::test_that("Check the BT_Relative_Influence function - Results - With surrogates",{
  ...
  expect_equal(relInf[BT_algo$var.names], BT_relative_influence(BT_algo, rescale = F, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfNormalized[BT_algo$var.names], BT_relative_influence(BT_algo, rescale = T, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfSorted, BT_relative_influence(BT_algo, rescale = F, sort.it = T, consider.surrogates = T))
  expect_equal(relInfSortedAndNormalized, BT_relative_influence(BT_algo, rescale = T, sort.it = T, consider.surrogates = T))
  ...
  expect_equal(relInf[BT_algo$var.names], BT_relative_influence(BT_algo, n.iter, rescale = F, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfNormalized[BT_algo$var.names], BT_relative_influence(BT_algo, n.iter, rescale = T, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfSorted, BT_relative_influence(BT_algo, rescale = F, n.iter, sort.it = T, consider.surrogates = T))
  expect_equal(relInfSortedAndNormalized, BT_relative_influence(BT_algo, n.iter, rescale = T, sort.it = T, consider.surrogates = T))
  ...
  expect_equal(relInf[BT_algo$var.names], BT_relative_influence(BT_algo, n.iter, rescale = F, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfNormalized[BT_algo$var.names], BT_relative_influence(BT_algo, n.iter, rescale = T, sort.it = F, consider.surrogates = T)[BT_algo$var.names])
  expect_equal(relInfSorted, BT_relative_influence(BT_algo, n.iter, rescale = F, sort.it = T, consider.surrogates = T))
  expect_equal(relInfSortedAndNormalized, BT_relative_influence(BT_algo, n.iter, rescale = T, sort.it = T, consider.surrogates = T))
