    ...
    for (k in 1:length(alpha)){
      ...
      for (m in 1:M){
        ...
        tree<-rpart(formula = modelForm,
                    data = trainingsubset,
                    method = "poisson",
                    control = rpart.control(cp=-Inf, maxdepth = depth, xval=0, minsplit = 2, maxcompete = 0, maxsurrogate = 0)) # cp = complexity parameter, maxdepth = maximum depth of any node of the final tree (root node counted as depth 0), xval=0: no cross validation

        ff <- as.data.frame(cbind("node"=as.numeric(rownames(tree$frame[tree$frame$var != "<leaf>", ])), "improve"=unname(tree$splits[, "improve"])))
        ...
        for (currSplit in seq(1, depth)){
          if (currSplit==1){
            ...
            if (all(is.na(indx))){
              ...
              children <- c(setdiff(children, bestNode), c(bestNode*2, bestNode*2+1))
            }
          }
        }
        if (!is.null(tree$splits) & length(setdiff(ff$node, nodeToKeep)) > 0){
          tree.opt <- snip.rpart(tree, setdiff(ff$node, nodeToKeep))
        }else{
          tree.opt <- tree
        }

        size.tree[m] <- (nrow(tree.opt$frame[tree.opt$frame$var != "<leaf>",]) == depth)
    ...
    for (k in 1:length(alpha)){
      ...
      for (m in 1:M){
        ...
        tree<-rpart(formula = modelForm,
                    data = trainingsubset,
                    method = "poisson",
                    control = rpart.control(cp=0, maxdepth = depth, xval=0, minsplit = 2, maxcompete = 0, maxsurrogate = 0)) # cp = complexity parameter, maxdepth = maximum depth of any node of the final tree (root node counted as depth 0), xval=0: no cross validation
...
testthat::test_that("Same results as MVP code - BT with CV - no bagging and colsample.bytree argument",{
  ...
  size.tree.matrix<-matrix(F,ncol=nFolds, nrow=M) # size (number of internal nodes) of the trees
  ...
  for (iFolds in seq(1, nFolds)){
    ...
    for (m in 1:M){
      ...
      size.tree.matrix[m, iFolds] <- (nrow(tree.opt$frame[tree.opt$frame$var != "<leaf>",]) == depth)
...
testthat::test_that("Same results as MVP code - BT with CV - WITH bagging and colsample.bytree argument + interaction.depth NULL",{
...
testthat::test_that("Check expected ID",{
  ...
  treesSize1 <- (sapply(seq(1, n.iter), function(xx){nrow(BT_algo1$BTIndivFits[[xx]]$frame[BT_algo1$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) == interaction.depth)

  interaction.depth <- 10
  paramsBT$interaction.depth <- interaction.depth
  BT_algo2 <- do.call(BT, paramsBT)
  treesSize2 <- (sapply(seq(1, n.iter), function(xx){nrow(BT_algo2$BTIndivFits[[xx]]$frame[BT_algo2$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) == interaction.depth)

  paramsBT$formula <- as.formula("Y_normalized ~ Age")
  BT_algo3 <- do.call(BT, paramsBT)
  treesSize3 <- (sapply(seq(1, n.iter), function(xx){nrow(BT_algo3$BTIndivFits[[xx]]$frame[BT_algo3$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) == interaction.depth)

  interaction.depth <- 7
  paramsBT$interaction.depth <- interaction.depth
  paramsBT$formula <- as.formula("Y_normalized ~ Sport + Split + Gender")
  BT_algo4 <- do.call(BT, paramsBT)
  treesSize4 <- (sapply(seq(1, n.iter), function(xx){nrow(BT_algo4$BTIndivFits[[xx]]$frame[BT_algo4$BTIndivFits[[xx]]$frame$var != "<leaf>",])}) == interaction.depth)
