...
############################
#' @keywords internal
iteration_error <- function(object, which=c('train', 'validation', 'cv')) {
...
check_tweedie_power <- function(tweedie.power){
  if (is.null(tweedie.power)) stop("Please defined a tweedie power.")
  if (!is.double(tweedie.power) || (length(tweedie.power) > 1) || is.infinite(tweedie.power)) stop("tweedie.power should be a finite numeric")
  if (tweedie.power > 0 && tweedie.power < 1) stop("tweedie.power is not defined between 0 and 1.")
}

#' @keywords internal
check_n_iter <- function(n.iter){
  if (is.null(n.iter) || !check_if_natural_number(n.iter) || (length(n.iter) > 1)) stop("n.iter should be a positive integer.")
}

#' @keywords internal
check_interaction_depth <- function(interaction.depth){
  if (!is.null(interaction.depth) && (!check_if_natural_number(interaction.depth) || (length(interaction.depth) > 1))) stop("When defined interaction.depth should be a positive integer.")
}

#' @keywords internal
check_shrinkage <- function(shrinkage){
  if (is.null(shrinkage) || (length(shrinkage) > 1)) stop("Please define a shrinkage parameter.")
  if (shrinkage <=0 || shrinkage > 1) stop("Shrinkage parameter should be > 0 and <=1.")
}

#' @keywords internal
check_bag_fraction <- function(bag.fraction){
  if (is.null(bag.fraction) || (length(bag.fraction) > 1)) stop("Please define a bag.fraction parameter.")
  if (bag.fraction <=0 || bag.fraction > 1) stop("bag.fraction parameter should be > 0 and <=1.")
}

#' @keywords internal
check_colsample_bytree <- function(colsample.bytree, numExplVar){
  if (!is.null(colsample.bytree)){
    if (length(colsample.bytree) > 1) stop("colsample.bytree should be a positive integer.")
    if (colsample.bytree>numExplVar) stop("colsample.bytree should be lower than the number of explanatory variables.")
    if (!check_if_natural_number(colsample.bytree)) stop("colsample.bytree should be a positive integer.")
  }
}

#' @keywords internal
check_train_fraction <- function(train.fraction){
  if (is.null(train.fraction) || (length(train.fraction) > 1)) stop("Please define a train.fraction parameter.")
  if (train.fraction <=0 || train.fraction >1) stop("When defined train.fraction should be > 0 and <= 1.")
}
...
check_cv_folds <- function(cv.folds){
  if (is.null(cv.folds)) stop("cv.folds should be defined.")
  if (!check_if_natural_number(cv.folds) || (length(cv.folds) > 1)) stop("cv.folds should be a positive integer.")
}
...
check_n_cores <- function(n.cores){
  if (!check_if_natural_number(n.cores) || (length(n.cores) > 1)) stop("n.cores should be a positive integer.")
}
...
#' @keywords internal
check_if_natural_number <- function(x, tol = .Machine$double.eps^0.5){
